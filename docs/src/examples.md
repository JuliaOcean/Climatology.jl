...
- [Ocean State Estimates](ECCO_standard_plots.html) (➭ [code link](https://raw.githubusercontent.com/JuliaOcean/Climatology.jl/master/examples/ECCO/ECCO_standard_plots.jl)) : explore ocean transports, climate indices, subsurface temperature, and many more variables from full ocean state estimates (ECCO, OCCA).
...
- OCCA2 : [Forget 2026](https://doi.org/10.5194/egusphere-2026-4643)
...
- [Sea Level Time Series](NSLCT_notebook.html) (➭ [code link](https://raw.githubusercontent.com/JuliaOcean/Climatology.jl/master/examples/NSLCT/NSLCT_notebook.jl)) : plot global mean and regional sea level data from NASA (NASA's NSLCT and ECCO). _(broken data source)_
- [NSLCT\_notebook.jl](NSLCT_notebook.html) lets you access sea level data from NASA and Dataverse portals (`HTTP.jl`, `Dataverse.jl`), organize it into tables (`DataFrames.jl`), and plot it (`Makie.jl`). _(broken data source)_
- examples in [OceanRobots.jl](https://juliaocean.github.io/OceanRobots.jl/dev/examples/) for observations collected at sea (in situ data).
