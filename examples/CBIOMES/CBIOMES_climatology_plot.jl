...
begin
	using OceanStateEstimation, MeshArrays, NCDatasets
	using Downloads, PlutoUI, JLD2, CairoMakie

	ff = @bind file_selection PlutoUI.NumberField(1:3, default=1)
...
md"""# CBIOMES Near Surface Climatologies

In this notebook we visualize ocean climatologies interactively.

The inputs to this notebook is the file generated by [that other notebook](https://gaelforget.github.io/OceanStateEstimation.jl/v0.1.13/examples/CBIOMES_model_climatogy.html), downloaded from [this zenodo archive](https://doi.org/10.5281/zenodo.5598417).

More information about this monthly, 1992-2011, climatology state estimate can be found [here](https://cbiomes.readthedocs.io/en/latest/).
...
begin
	CBIOMESclim_path=ScratchSpaces.CBIOMES
	if file_selection == 1
		fil_out=joinpath(CBIOMESclim_path,"CBIOMES-global-alpha-climatology.nc")
		!isfile(fil_out) ? OceanStateEstimation.CBIOMESclim_download() : nothing
		if isdir(fil_out) #temporary fix
			mv(fil_out,fil_out*"_tmp")
			mv(joinpath(fil_out*"_tmp","CBIOMES-global-alpha-climatology.nc"),fil_out)
		end
	elseif file_selection == 2
		fil0="gridded_darwin_montly_clim_360_720_ver_0_2_6.nc"
		url="https://rsg.pml.ac.uk/shared_files/brj/CBIOMES_ecoregions/ver_0_2_5/"*fil0
		fil_out=joinpath(CBIOMESclim_path,fil0)
		!isfile(fil_out) ? Downloads.download(url,fil_out) : nothing
	elseif file_selection == 3
		fil0="gridded_geospatial_montly_clim_360_720_ver_0_2.nc"
		url="https://rsg.pml.ac.uk/shared_files/brj/CBIOMES_ecoregions/ver_0_2_6/"*fil0
		fil_out=joinpath(CBIOMESclim_path,fil0)
		!isfile(fil_out) ? Downloads.download(url,fil_out) : nothing
	end
	...
	"WindSpeed" => 20.0*[0.0:0.1:1.0],
		#for gridded_geospatial_montly_clim_360_720_ver_0_2.nc (values seem odd) 
	"wind" => 20.0*[0.0:0.1:1.0], "EKE" => 0.1*[0.0:0.1:1.0],
	"mld" => 500.0*[0.0:0.05:1.0], "euphotic_depth" => 0.1*[0.0:0.1:1.0], 
	"Kd490" => 0.1*[0.0:0.1:1.0])
end
...
begin
	...
	vv = @bind v PlutoUI.Select(v_lst, default="Chl")
...
let
	fig = Figure(resolution = (600,400), backgroundcolor = :grey95, fontsize=12)
	
	ax = Axis(fig[1,1], title=nam*" (units: $(uni) ; month: $(m))",xlabel="longitude",ylabel="latitude")
	if haskey(levels,v)
		lev=levels[v][1]
	...
	hm1=contourf!(ax,lon,lat,val, tickfont = (4, :black), levels = lev)
	#
	lon1=[lon0-5 lon0+5 lon0+5 lon0-5 lon0-5]
	lat1=[lat0-5 lat0-5 lat0+5 lat0+5 lat0-5]
	lines!(ax,lon1[:],lat1[:],color=:red,linewidth=3.0)
	#
...
let
	...
	fig = Figure(resolution = (600,300), backgroundcolor = :grey95, fontsize=12)

	ax = Axis(fig[1,1], title=nam*" (units: $(uni) ; month: $(m))",xlabel="month",ylabel="latitude")
	hm2=contourf!(ax,0.5:35.5,lat[j2], transpose([tmp tmp tmp]),levels=lev)
	Colorbar(fig[1,2], hm2, height = Relative(0.65))
...
function regional_mean(lon0,lat0,v)
	...
	val_region[findall(ismissing.(val_region))].=NaN
...
let
	...
	fig = Figure(resolution = (900,600), backgroundcolor = :grey95, title="lon0,lat0=$(lon0),$(lat0)" , fontsize=12)
	
	if file_selection == 1
		w_lst=vec(["Chl" "MLD" "SST" "SSS" "PAR" "TKE" "EuphoticDepth" "WindSpeed" ])
	elseif file_selection == 2
		w_lst=vec(["Chl" "MLD" "SST" "SALT" "PAR" "TKE" "euphotic_depth" "wind" ])
	elseif file_selection == 3
		w_lst=vec(["Chl" "mld" "SST" "Kd490" "PAR" "EKE" "euphotic_depth" "wind" ])
	end
	...
	for w in vec(w_lst[:])
		...
		ax = Axis(fig[jj,ii], title=M.n,xlabel="month",ylabel="$(M.u)")
		lines!(ax,M.x,M.y,color=:blue,linewidth=2.0); xlims!(ax, (0.0, 12.0))
	end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "d5f6007e0b7c9c4c8b960d219a7551520e9c121f"
