...
begin
	using PlutoUI
	import MeshArrays, AirSeaFluxes, Climatology
	import NCDatasets, DataFrames, Interpolations
	import CSV, Statistics
	import CairoMakie, RollingFunctions
	
	isdefined(Main,:ERA5_read) ? nothing : include("SurfaceFluxes_modules.jl")

	import .ERA5_read: interpolate_sst, surface_balance
	import .ERA5_plot: plot_bulk_formulae, plot_Qnet_cumsum, plot_surface_balance

	"done with packages"

end

# ╔═╡ 0165b0cc-049c-40af-bf67-c9957338d407
readdir(Climatology.downloads.ERA5_OISST_download())

# ╔═╡ cb3043b0-7d05-4ce7-853e-1c2097c4a910
md"""# Air-Sea Fluxes
...
# ╔═╡ 7c79c4db-dd86-4e71-9909-d9410be71b9d
@bind doSave CheckBox(default=false)
...
begin
	...
	ERA5=CSV.read(joinpath(path_to_data,fil_ERA5),DataFrames.DataFrame)
	OISST=CSV.read(joinpath(path_to_data,fil_OISST),DataFrames.DataFrame)
	
	tim=(1:365*24)./24 .-0.5/24
	sst=interpolate_sst(OISST.sst,tim)
		
	df=surface_balance(ERA5,sst)
end
...
md"""## Bulk Formulae alone"""

# ╔═╡ 9cea378c-c24b-11f0-aae7-1579b1b9972b
begin
	path_to_nc="/Volumes/My Passport/data/ERA5"
	fil=joinpath(path_to_nc,"2023","ERA5_2023_01.nc")
	#fil=joinpath("..","data","ERA5_2023_01.nc")

	ispath(fil) ? plot_bulk_formulae(fil,15) : "file not found"
end
...
md"""The ERA5 was created on engaging using `Climatology.jl` (see Appendix)

```
let
	using DataFrames
	CSV=Climatology.SST_FILES.CSV
	
	tim=collect(1:length(ts.anom))/365.25 .+ 1982
#	lines(tim,ts.sst)

	ii=findall(ts.year.==2023)
	ds=DataFrame(tim=tim[ii],sst=ts.sst[ii])
#	lines(ds.tim,ds.sst)

	CSV.write(joinpath(tempdir(),"OISST_2023.csv"),df)
...
manifest_format = "2.0"
project_hash = "c0010a5253003436f377193b155ebab21b429aad"
